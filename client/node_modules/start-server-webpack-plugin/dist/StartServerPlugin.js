...
class StartServerPlugin {
  ...
  _getScript(compilation) {
    ...
    if (!entry) {
      ...
      throw new Error(`Requested entry "${entryName}" does not exist, try one of: ${(map.keys ? map.keys() : Object.keys(map)).join(' ')}`);
    }
    ...
    return _path2.default.resolve(path, entryScript);
  }
  ...
  _runWorker(callback) {
    ...
    const worker = _child_process2.default.fork(scriptFile, args, { execArgv });
